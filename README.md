...
首次运行前，在 `Model_Navigate/data/` 目录下放置基线 Excel 表格：

- **`Object-Models-Old.xlsx`** — 基线表格（已追踪的模型列表）
- **`Object-Models.xlsx`** — 原始基线（只读备份）

可以从空表开始，也可以使用已有模型数据。表格需包含 14 列：

```
模型名称 | 是否接入 | workflow接入进展 | 公司 | 国内外 | 开闭源 | 尺寸 | 类型 | 是否推理 | 任务类型 | 官网 | 备注 | 发布时间 | 记录创建时间
```
