...
项目内置金标评估能力，用于量化流水线产出质量。

### 金标文件

从 `Object-Models.xlsx` 中提取指定时间段的记录作为金标（ground truth），当前金标为 2026/01/01~2026/03/30 共 116 条。

### 评估指标

| 指标 | 说明 |
|------|------|
| 覆盖率 (Recall) | 金标中的模型有多少被流水线找到 |
| 字段准确率 | 匹配模型中各字段的一致性 |
...
> **关于第三源 HuggingFace**：HuggingFace API 作为补充数据源，主要用于交叉校验已采集模型的参数量、发布日期等字段，并补全各类开源模型的变体版本（如不同参数规格的 Qwen3.5 系列）。它不参与覆盖率统计，但显著提升了字段准确率和数据完整度。

### 运行评估

```bash
cd Model_Navigate/Eval
python 20260509_1347_eval_combined.py
```
