...
5. **核实时必须填写模型发布时间**：在核实每个模型时，同步查找并记录该模型的首次公开发布日期

#### 模型发布时间填写规则

"模型发布时间"列位于"备注"和"记录创建时间"之间，记录模型的**首次公开发布/公告日期**（非录入日期）。

| 规则 | 说明 | 示例 |
|------|------|------|
| **格式** | `YYYY-MM-DD`，精确到日 | `2026-04-15` |
| **来源优先级** | 官方博客/公告 > 新闻报道 > 腾讯研究院文章日期 > 推断 | — |
| **无法确定精确日期** | 使用文章发布日期（腾讯研究院AI速递的日期），并在备注中标注"发布时间为文章日期" | `2026-04-16`（文章日期） |
| **完全无法确定** | 留空（None），不猜测 | — |
| **已有模型补填** | 后续核实时可回填，不强制首次录入时填写 | — |
| **区分发布与录入** | "模型发布时间"≠"记录创建时间"；前者是模型面世日期，后者是录入表格日期 | — |

**核实时的操作**：在 web_search / HuggingFace / 官网核实模型信息时，同步查找发布日期，填入 `NEW_MODELS` 字典的 `"模型发布时间"` 字段。
