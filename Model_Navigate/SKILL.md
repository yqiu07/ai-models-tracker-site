...
如果你在**阿里巴巴内网**（办公电脑 + 阿里郎）环境中使用，外网 API（如 `api.kuai.host`）可能被**云壳安全策略**拦截。

**解决方法**：在云壳后台将 API 域名加白：
1. 打开 **云壳 → 防护记录 → 域名拦截**
2. 找到被拦截的域名（如 `api.kuai.host`），点击右侧按钮申请加白
3. 加白后重新运行即可

**症状**：`SSLError: certificate verify failed: self-signed certificate in certificate chain`，或 403 返回"网站不在安全策略默认允许的范围内"。
