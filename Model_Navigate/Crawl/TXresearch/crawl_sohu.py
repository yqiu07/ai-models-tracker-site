...
def create_driver():
    """创建 headless Chrome WebDriver 实例。"""
...
def _parse_single_card(element) -> dict | None:
    """解析单个搜狐号文章卡片元素，返回文章信息 dict。

    返回字段: title, summary, link, publish_time, read_count, comment_count
    解析失败返回 None。
    """
    try:
        # 标题 + 链接
        title_element = None
        for selector in ["h4 a", "h3 a", "a.title", "a[href*='sohu.com']"]:
            try:
        ...
        # 简介
        summary = ""
        for desc_sel in ["p.desc", "p.summary", "div.desc", "p"]:
            try:
                desc_el = element.find_element(By.CSS_SELECTOR, desc_sel)
        ...
        # 发布时间
        publish_time = ""
        for time_sel in ["span.time", "span.date", "time", "span.publish-time"]:
            try:
                time_el = element.find_element(By.CSS_SELECTOR, time_sel)
                publish_time = time_el.text.strip()
                if publish_time:
                    break
            except Exception:
                continue
...
def scroll_and_collect(driver, since_int: int, until_int: int):
    ...
    # 连续超出范围计数器：搜狐号页面文章不严格按时间倒序排列，
    # 评论文章与速递穿插，单篇 < since 不能立即停止。
    # 需要连续 CONSECUTIVE_OLD_THRESHOLD 篇有日期且 < since 才判定为翻到头了。
    CONSECUTIVE_OLD_THRESHOLD = 3
        ...
        for article in current_articles:
            ...
            if article_date:
                ...
                if article_date_int < since_int:
                    ...
                    print(f"[Old {consecutive_old_count}/{CONSECUTIVE_OLD_THRESHOLD}] "
                          f"'{article['title'][:40]}' "
                          f"日期={article_date_int} < since={since_int}")
                    if consecutive_old_count >= CONSECUTIVE_OLD_THRESHOLD:
                        print(f"[Stop] 连续 {CONSECUTIVE_OLD_THRESHOLD} 篇超出范围，停止爬取")
                        reached_before_since = True
                        break
                elif article_date_int > until_int:
                    # 文章日期晚于截止时间 → 跳过，继续滚动
                    consecutive_old_count = 0  # 重置：遇到新文章说明还没翻到头
