...
- 如果文中明确写了"X月X日发布/上线/开源" → 用该日期
- 如果文中写"昨日/今日/近日" → 结合文章日期推算
- 如果完全无法确定具体发布日期 → `release_date` 留空字符串 ""
- **绝对禁止**：把文章日期直接当作发布日期填入
...
- release_date: 实际发布日期（YYYY-MM-DD）。必须从正文中找到明确时间锚点。如无明确时间则**留空**。
- release_date_source: 日期来源，取值为 "文中明确提及" / "根据上下文推断" / ""（留空表示未找到）
...
示例：
[
  {{"model_name": "Claude Opus 4.7", "company": "Anthropic", "domestic": "国外", "open_source": "闭源", "size": "", "model_type": "基座", "can_reason": "thinking", "task_type": "通用对话", "release_date": "2026-04-15", "release_date_source": "文中明确提及", "website": "https://claude.ai", "brief": "Anthropic旗舰模型"}},
  {{"model_name": "Qwen3.6-27B", "company": "阿里", "domestic": "国内", "open_source": "开源", "size": "27B", "model_type": "基座", "can_reason": "thinking", "task_type": "通用对话,代码生成", "release_date": "", "release_date_source": "", "website": "https://github.com/QwenLM/Qwen3", "brief": "阿里开源27B推理模型"}}
]
...
def load_articles(since_int: int | None = None, until_int: int | None = None) -> list[dict]:
    ...
    for txt_file in sorted(ARTICLES_DIR.glob("*.txt")):
        ...
        # 时间筛选：有 since/until 参数时，排除无日期或日期范围外的文章
        if since_int or until_int:
            if article_date_int == 0:
                # 标题中无日期信息，无法判断时间范围，跳过
                continue
            if since_int and article_date_int < since_int:
                continue
            if until_int and article_date_int > until_int:
                continue
